    ...
    public class AccountService : IAccountService
    {
        ...
        public async Task<LoginResponse> LoginAsync(LoginRequest request)
        {
            ...
            try
            {
                ...
                if (user==null)
                {
                    user = await signInManager.UserManager.FindByEmailAsync(request.Username).ConfigureAwait(false);
                }
                ...
                if (result.Succeeded)
                {
                    ...
                    response.MainObjectId = user.MainObjectId?.ToString();
            ...
            return await executionStrategy.Execute(async () =>
            {
                ...
                try
                {
                    ...
                    if (result.Succeeded)//Add user to a role
                    {
                        ...
                        var addUserToRoleResult = await AddUserToRole(user, request.RoleName).ConfigureAwait(false);
                        ...
                        //throw new Exception(addUserToRoleResult.Message);
                    }
                    else
                    {
                        if (result.Errors!=null && result.Errors.ToArray().Length>0)
                        {
                            response.HttpStatusCode = System.Net.HttpStatusCode.OK;
                            response.Succeeded = false;//addUserToRoleResult.Succeeded;
                            response.Message = result.Errors.ToArray()[0].Description;
                        }
                    }
        ...
        public async Task<ResponseModel> ChangePhoneNumberAsync(ChangePhoneNumberRequest request)
        {
            ...
            var passIsCorrect = await signInManager.UserManager.CheckPasswordAsync(user, request.Password);
            if (!passIsCorrect)
            {
            ...
            var token = await signInManager.UserManager.GenerateChangePhoneNumberTokenAsync(user, user.PhoneNumber);
            logger.Debug("PhoneNumber is:" + user.PhoneNumber + "-" + token);
            // Send token via SMS
            if (!string.IsNullOrEmpty(token))
            {

                MB_MessageSend mB_MessageSend = new MB_MessageSend();
                var messageSendModel = new MessageSendModel()
                {
                    Body = $"Touba Verification Code: {token}",
                    Receiver = user.PhoneNumber,
                    UniCode = "UTF"
                };
               
                var smsMessageResponse = await messageService.SendAsync(messageSendModel);


                logger.Debug("res:" + request.Password);
                response.Succeeded = smsMessageResponse.IsSend;
        ...
        public async Task<ResponseModel> ConfirmPhoneNumberAsync(ConfirmPhoneNumberRequest request)
        {
            ...
            var res = await signInManager.UserManager.ChangePhoneNumberAsync(user, user.PhoneNumber, request.Token);
            if (!res.Succeeded)
            {
        ...
        public async Task<ResponseModel> CreateUserAsync(CreateUserByAdminRequest request)
        {
            ...
            if (await signInManager.UserManager.FindByNameAsync(request.UserName) != null)
            {
            ...
            return await executionStrategy.Execute(async () =>
            {
                ...
                try
                {
                    ...
                    {
                        var res = await signInManager.UserManager.AddToRoleAsync(user, item);
                        if (!res.Succeeded)
                        {
        ...
        public async Task<ResponseModel> ConfirmEmailAsync(string token, string email)
        {
            ...
            try
            {
                ...
                var user = await signInManager.UserManager.FindByEmailAsync(email).ConfigureAwait(false);
                if (user == null)
                {
                    response.Message = AccountOptions.EmailAddressIncorrect;
                    return response;
                }
                var result = await signInManager.UserManager.ConfirmEmailAsync(user, token).ConfigureAwait(false);
                if (result.Succeeded)
                {
        ...
        public async Task<ResponseModel> GenerateForgetPasswordTokenAsync(ForgetPasswordRequest request)
        {
            ...
            try
            {
                ...
                User user = null;
                if (!string.IsNullOrEmpty(request.Email))
                    user = await signInManager.UserManager.FindByEmailAsync(request.Email.Trim()).ConfigureAwait(false);
                else
                    user = await signInManager.UserManager.FindByNameAsync(request.Username.Trim()).ConfigureAwait(false);
        ...
        public async Task<UserProfileResponse> GetUserProfile(string userId)
        {
            ...
            // Get user roles
            var userRoles = await signInManager.UserManager.GetRolesAsync(user).ConfigureAwait(false);
        ...
        public async Task<ResponseModel> ResetPasswordAsync(ResetPasswordModel request)
        {
            ...
            try
            {
                ...
                if (user != null)
                {
                    var result = await signInManager.UserManager.ResetPasswordAsync(user, request.Token, request.Password).ConfigureAwait(false);
                    if (result.Succeeded)
                    {
                        response.Succeeded = result.Succeeded;
                        response.HttpStatusCode = System.Net.HttpStatusCode.OK;
        ...
        public async Task<ResponseModel> EditRole(EditRoleNameRequest editRole)
        {
            ...
            if (await roleManager.RoleExistsAsync(editRole.RoleName))
            {
        ...
        public async Task<List<UserResponse>> GetRoleUsersAsync(GetRoleUsersRequest request)
        {
            ...
            var filteredUsers = users.Where(f => f.IsActive == true).ToList();
        ...
        public async Task<ResponseModel<GetAllUsersResponse>> GetAllUsersAsync() => new ResponseModel<GetAllUsersResponse>
        {
            Succeeded = true,
            HttpStatusCode = System.Net.HttpStatusCode.OK,
            ResponseList = await context.Users.Select(x => new GetAllUsersResponse { Id = x.Id, UserName = x.UserName }).ToListAsync()
        };
        ...
        public async Task<MB_GetUserInfoByIdResponse> GetUserInfoAsync(string UserId)
        {
            ...
            return new MB_GetUserInfoByIdResponse
            {
                ...
                MainObjectId=user.MainObjectId
            };
        }
        ...
        private async Task<IEnumerable<Claim>> GetUserClaims(User user) => await signInManager.UserManager.GetClaimsAsync(user).ConfigureAwait(false);
        private async Task<ResponseModel> AddUserToRole(User user, string roleName)
        {
            var response = new ResponseModel
            {
                Succeeded = true
        ...
        public async Task<LoginResponse> GetAccessToken(User user, string secretCode)
        {
            ...
            var roles = await userManager.GetRolesAsync(user);
            var role = roles.Any() ? roles.First() : Roles.Customer;
            var tokenDescriptor = new SecurityTokenDescriptor
            {
